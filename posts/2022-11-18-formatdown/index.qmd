...
#| echo: false
#| label: tbl-examples
#| tbl-cap: Rendering a number using different formats
Notation <- c("without $10^n$", "scientific", "engineering")
Name <- c("`x1`", "`x2`", "`x3`")
Value <- c("`$0.0004567$`", 
           "`$4.567\\times{10}^{-4}$`", 
           "`$456.7\\times{10}^{-6}$`")
Rendered_as <- c(x1, x2, x3)

DT <- data.table(Notation, Name, Value,  Rendered_as)
knitr::kable(DT, align = "lcrr")
```
...
which is rendered (in this output document) as $N_A =$ `r format_power(6.0221E+23, digits = 5)`. 
